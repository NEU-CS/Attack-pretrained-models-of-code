...
class Attacker():
    ...
    def greedy_attack(self, code,true_label,subs:dict,ifreplace = True,ifinsert = True,beam_size = 1,sort = False):
        '''
        return
            original program: code
            program length: prog_length
        ...
        acc_nums = 0
        max_sub_size = max([len(l) for l in list(subs.values())])
        
        dp_prob = [[1 for _ in range(max_sub_size + 1 )] for _ in range(len(sorted_list_of_names))]  #prob最大不会大于1，初始化为1
        dp_code = [["" for _ in range(max_sub_size + 1)] for _ in range(len(sorted_list_of_names))] #dp_code[i][j]表示应用到此变换最小prob对应的代码
        '''
        总的公式: dp[i][j] = min( M(Transform(dp_code[i-1][j],a[i][j] ) ) ). 其中a[i][j]表示某种变换,包括但不限于重命名变量,插入死代码等等。反正a[i][j]会对代码进行semantic-preserved transformation        
        '''
        '''
        对variable进行重要性排序有没有用尚未研究
        '''
        
        
        if ifreplace:
            #以下是对index等于0的初始化
            name_and_score = sorted_list_of_names[0]
            tgt_word = name_and_score[0]
            all_substitues = subs[tgt_word]
            dp_prob[0][0] = current_prob
            dp_code[0][0] = final_code
            for j in range(1,len(all_substitues)+1):
                dp_code[0][j] = get_example(final_code,tgt_word,all_substitues[j-1],"python")
                temp_result = self.classifier.predict(dp_code[0][j])
                label = temp_result[0]['label']
                prob = temp_result[0]['score']
                if label!= true_label:
                    print("SUC!",tgt_word,"=>",all_substitues[j-1],"prob:",prob,sep = " ")
                    #这里的返回值等会再check
                    return code, prog_length, dp_code[0][j], true_label, orig_label, label, 1, variable_names, importance_score, nb_changed_var, nb_changed_pos, replaced_words,"replace",[],sum([sum(a) for a in sub_most_indexs]),acc_nums
                dp_prob[0][j] = prob

            #index大于等于1
            for i in range(1,len(sorted_list_of_names)):
                name_and_score = sorted_list_of_names[i]
                tgt_word = name_and_score[0]
                all_substitues = subs[tgt_word]
                    
                #对不进行替换的最优子结构传播，直接使用之前的结果，以免重复计算
                for k in range(max_sub_size + 1):
                    if dp_prob[i-1][k] < dp_prob[i][0] :
                        dp_prob[i][0] = dp_prob[i-1][k]
                        dp_code[i][0] = dp_code[i-1][k]
                    
                #对进行替换的最优子结构传播
                for j in range(1,len(all_substitues)+1):
                    codes_transform = []
                    for k in range(max_sub_size + 1):
                        if dp_prob[i-1][k] >= 1 or dp_code == "":
                            continue
                        code = get_example(dp_code[i-1][k],tgt_word,all_substitues[j-1],"python")
                        codes_transform.append(code)
                    #先存入一个list,为了并行计算，效率更高
                    temp_result = self.classifier.predict(codes_transform)
                    
                    for oneresult in temp_result:
                        label = oneresult['label']
                        prob = oneresult['score']
                        if label!= true_label:
                            print("SUC!",tgt_word,"=>",all_substitues[j-1],"prob:",prob,sep = " ")
                                #这里的返回值等会再check
                            return code, prog_length, code, true_label, orig_label, label, 1, variable_names, importance_score, nb_changed_var, nb_changed_pos, replaced_words,"replace",[],sum([sum(a) for a in sub_most_indexs]),acc_nums
        ...
        if ifinsert:
            ...
            for addcode in vocab:
                ...
                if temp_result[0]['label'] != true_label:
                    ...
                    is_success = 1
                    return code, prog_length, adv_code, true_label, orig_label, temp_result[0]['label'] , is_success, variable_names, importance_score, nb_changed_var, nb_changed_pos, replaced_words,"insert",insert_words,sum([sum(a) for a in sub_most_indexs]),acc_nums
                else:
        ...
        return code, prog_length, adv_code, true_label, orig_label, orig_label, is_success, variable_names, importance_score, nb_changed_var, nb_changed_pos, replaced_words,"failed",insert_words,sum([sum(a) for a in sub_most_indexs]),acc_nums
