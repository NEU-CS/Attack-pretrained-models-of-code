...
# @File    : attacker.py
'''For attacking DeepSeek models for Codetrans with vllm'''
import sys
import os
...
from transformers import AutoModelForSequenceClassification,pipeline,AutoTokenizer,RobertaForMaskedLM,RobertaTokenizer
...
def main():
    ...
    parser.add_argument("--model_name_or_path", default="deepseek-ai/deepseek-coder-6.7b-base", type=str,
                        help="The model checkpoint for weights initialization.")
    parser.add_argument("--tokenizer_name", default="deepseek-ai/deepseek-coder-6.7b-base", type=str,
                        help="The model checkpoint for weights initialization.")
    ...
    parser.add_argument("--sort_substitute",action="store_true")
    ...
    ## Load Target Model
    tokenizer = AutoTokenizer.from_pretrained(args.tokenizer_name,model_max_length = 1024 , pad_token = "<|pad|>")
    ...
    for index, example in enumerate(eval_dataset):
        ...
        subs = substs[index]
        print("enter")
        code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words,Suctype,insertwords,most_index,acc_nums,original_prob,current_prob = attacker.greedy_attack(code,true_label, subs,args.use_replace,args.use_insert)
        
        all_acc_nums += acc_nums
        greedy_query_times = classifier.query() - query_times
        all_sub_index += most_index
        ganb_changed_var = 0
        ganb_changed_pos = 0
        attack_type = "Greedy"
        ...
        print("Greedy sub index:",most_index)
        print("Ga query times:",ga_query_times)
        print("All Query times: ", classifier.query())
        print("All sub index:",all_sub_index)
        print("Avg sub index",all_sub_index//all_acc_nums )
        ...
        print("Index: ", index)
        print()
        recoder.write(index, code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, score_info, nb_changed_var, nb_changed_pos, replace_info, ganb_changed_var,ganb_changed_pos,attack_type, classifier.query() - query_times, example_end_time,Suctype,insertwords,original_prob,current_prob)
