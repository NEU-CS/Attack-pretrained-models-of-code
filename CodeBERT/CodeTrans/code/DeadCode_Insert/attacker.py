...
def compute_fitness(chromesome,classifier, orig_prob ,code):
    ...
    result = classifier.predict({'code':temp_code})
    # 计算fitness function
...
def get_importance_score(words_list: list,variable_names: list, classifier,orig_prob,true_label):
    ...
    for variable,code_masked in new_example.items():
        results[variable] = classifier.predict({'code':code_masked,'label':true_label})[0]['score']
...
class Attacker():
    ...
    def insert_attack(self,pos:int,source_code:str,var_tgt,candidate,insert_deadcode:str,true_label):
            all_insert_pos = self.get_program_stmt_nums(source_code)
            code = all_insert_pos[:pos] + [insert_deadcode] + all_insert_pos[:pos]
            code = "".join(code)
            temp_result = self.classifier.predict({'code':code,'label':true_label})
            return temp_result
    
    def repalce_attack(self,pos,source_code:str,var_tgt:str,candidate:str,insert_deadcode,true_label):
    ...
    def greedy_attack(self, code,true_label,subs,ifreplace,ifinsert):
        ...
        results = self.classifier.predict({"code":code,"label":true_label})
        if type(results) == dict:
            results = [results]
        orig_label = results[0]['code']
        current_prob = results[0]['score']
        ...
        if len(variable_names) == 0:
            # 没有提取到identifier，直接退出
            is_success = -3
            return code, prog_length, adv_code, true_label, orig_label, orig_label, is_success, variable_names, None, None, None, None,None,None,origin_prob,origin_prob
        ...
        if importance_score is None:
            return code, prog_length, adv_code, true_label, orig_label, orig_label, -3, variable_names, None, None, None, None,None,None,origin_prob,origin_prob
        ...
        for iter in range(iter_nums):
            ...
            if transf == "replace":
                ...
                temp_results = self.classifier.predict({'code':temp_inputs,'label':true_label})
                lowest_index = 0
                ...
                if(len(ids) >= 2):
                    ...
                    for onedeadcode in deadcodes:
                        temp_results = self.insert_attack(insert_pos,final_code,"","",onedeadcode,true_label)
        ...
        return code, prog_length, final_code, true_label, orig_label, temp_code, is_success, variable_names, importance_score, nb_changed_var, nb_changed_pos, replaced_words,"failed",insert_words,origin_prob,current_prob
